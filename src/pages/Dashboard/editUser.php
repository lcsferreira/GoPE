...
<body>
      ...
      <form action="" method="post" id="userForm">
        ...
        <div class="form-group">
          <div class="switch-container">
            ...
            <input type="hidden" name="active-value" id="active-value"
              value="<?php echo $user['active'] === '1' ? 'true' : 'false'; ?>">
          </div>
        </div>
        <? if($user['active'] == 1 && $user['password'] != null) : ?>
        <div class="form-group">
          <label for="password" style="color: red">User already created a password, do wish to clear it?</label>
          <button type="button" id="clear-password"
            style="background-color: red; color: white; padding: 10px 20px; border: none; border-radius: 5px;">Clear</button>
        </div>
        <? endif; ?>
        <div>
          <? if($user['active'] == 1 && $user['password'] == null) {
            echo "<p><strong>User has not created a password!</strong> If any problem with contact activation email, you can copy and paste or click on the link below. </p>
            <p style='color: blue; text-decoration: underline;word-break: break-all'><a href='http://work.globalphysicaleducationobservatory.com/src/pages/Login/firstAccess.php?tk=".$user_id."'>http://work.globalphysicaleducationobservatory.com/src/pages/Login/firstAccess.php?tk=".$user_id."</a></p>";
          }
          ?>
        </div>
        <div class="country-relation-container">
          ...
          <div class="country-relation" id="country-relation-<?php echo $index + 1; ?>">
            ...
            <div class="switch-container">
              ...
              <!-- Rounded switch -->
              <label class="switch">
                <input type="checkbox" name="main-user" id="main-user-<?php echo $index + 1; ?>"
                  <?php echo $countryRelated['is_main'] == '1' ? 'checked' : ''; ?>>
                <span class="slider round"></span>
              </label>
              <!-- Hidden input field to store checkbox value -->
              <input type="hidden" name="main-user-value" id="main-user-value-<?php echo $index + 1; ?>"
                value="<?php echo $countryRelated['is_main'] === '1' ? 'true' : 'false'; ?>">
            </div>
  ...
  document.getElementById('clear-password')?.addEventListener('click', () => {
    clearPassword(<?php echo $user_id; ?>);
  });

  function clearPassword(user_id) {
    $.ajax({
      type: "POST",
      url: "../../query/Dashboard/clearPassword.php",
      data: {
        id: <?php echo $user_id; ?>
      },
      dataType: "text",
